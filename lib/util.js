...
async function loadCategory(categoryData, guild) {
    return new Promise((resolve) => {
        guild.channels.create({ name: categoryData.name, type: discord_js_1.ChannelType.GuildCategory }).then(async (category) => {
        ...
        const loadMessages = (channel, messages, previousWebhook) => {
            return new Promise(async (resolve) => {
                ...
                for (const msg of messages) {
                    const files = msg.files.map((file) => new discord_js_1.AttachmentBuilder(file.data, { name: file.name }));
